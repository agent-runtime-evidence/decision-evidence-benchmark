"""Validate and merge a reviewed manuscript source-review slice."""

from __future__ import annotations

import argparse
import csv
import json
from collections import Counter
from pathlib import Path
from typing import Any

from export_manuscript_source_review_workbook import CSV_COLUMNS as WORKBOOK_COLUMNS
from import_manuscript_source_review_workbook import _row_issues, read_jsonl

ROOT = Path(__file__).resolve().parents[1]
DEFAULT_SOURCE_ROOT = ROOT / "data/sources/manuscript_corpus"
DEFAULT_EXPECTED_SLICE = ROOT / "data/results/manuscript_source_review_slice.csv"
DEFAULT_REVIEWED_SLICE = ROOT / "data/results/manuscript_source_review_slice.reviewed.csv"
DEFAULT_BASE_WORKBOOK = ROOT / "data/results/manuscript_source_review_workbook.csv"
DEFAULT_MERGED_WORKBOOK = (
    ROOT / "data/results/manuscript_source_review_workbook.merged_from_slice.csv"
)
DEFAULT_REPORT = ROOT / "data/results/manuscript_source_review_slice_validation.json"
CASE_SOURCE_NAME = "case_evidence_sources.jsonl"

RESULT_HONESTY = (
    "The slice validator checks only the reviewed slice rows. The merge output is "
    "a full workbook authoring aid; it does not import source rows or promote "
    "manuscript evidence."
)


def read_csv(path: Path) -> list[dict[str, str]]:
    with path.open(newline="") as handle:
        return [dict(row) for row in csv.DictReader(handle)]


def validate_slice(
    *,
    source_rows: list[dict[str, Any]],
    expected_slice_rows: list[dict[str, str]],
    reviewed_slice_rows: list[dict[str, str]],
    base_workbook_rows: list[dict[str, str]] | None = None,
) -> dict[str, Any]:
    issues: list[dict[str, Any]] = []
    source_by_case_id = {str(row.get("case_id", "")): row for row in source_rows}
    expected_case_ids = [str(row.get("case_id", "")) for row in expected_slice_rows]
    reviewed_case_ids = [str(row.get("case_id", "")) for row in reviewed_slice_rows]

    issues.extend(_case_id_issues(expected_case_ids, reviewed_case_ids))

    expected_by_case_id = {
        str(row.get("case_id", "")): row for row in expected_slice_rows
    }
    for row_index, row in enumerate(reviewed_slice_rows, start=1):
        case_id = str(row.get("case_id", ""))
        if case_id in expected_by_case_id:
            issues.extend(
                _slice_row_identity_issues(
                    row=row,
                    expected_row=expected_by_case_id[case_id],
                    row_index=row_index,
                )
            )
        for issue in _row_issues(row, source_by_case_id=source_by_case_id):
            issues.append({**issue, "slice_row_index": row_index})

    if base_workbook_rows is not None:
        issues.extend(
            _base_workbook_issues(
                source_rows=source_rows,
                base_workbook_rows=base_workbook_rows,
            )
        )

    issue_counts = Counter(str(issue["issue"]) for issue in issues)
    return {
        "artifact_kind": "decision_evidence_manuscript_source_review_slice_validation",
        "valid": not any(issue["severity"] == "error" for issue in issues),
        "source_row_count": len(source_rows),
        "expected_slice_row_count": len(expected_slice_rows),
        "reviewed_slice_row_count": len(reviewed_slice_rows),
        "base_workbook_row_count": (
            len(base_workbook_rows) if base_workbook_rows is not None else None
        ),
        "selected_case_ids": expected_case_ids,
        "reviewed_case_ids": reviewed_case_ids,
        "review_status_counts": dict(
            sorted(
                Counter(
                    str(row.get("review_status", "")) for row in reviewed_slice_rows
                ).items()
            )
        ),
        "issue_counts": dict(sorted(issue_counts.items())),
        "issues": issues,
        "merge_written": False,
        "result_honesty": RESULT_HONESTY,
    }


def merge_slice_into_workbook(
    *,
    base_workbook_rows: list[dict[str, str]],
    reviewed_slice_rows: list[dict[str, str]],
) -> list[dict[str, str]]:
    reviewed_by_case_id = {
        str(row.get("case_id", "")): row for row in reviewed_slice_rows
    }
    merged_rows: list[dict[str, str]] = []
    for row in base_workbook_rows:
        case_id = str(row.get("case_id", ""))
        if case_id not in reviewed_by_case_id:
            merged_rows.append(_workbook_row(row))
            continue
        reviewed_row = reviewed_by_case_id[case_id]
        merged = _workbook_row(row)
        for field in WORKBOOK_COLUMNS:
            if field == "row_index":
                continue
            if field in reviewed_row:
                merged[field] = str(reviewed_row.get(field, ""))
        merged_rows.append(merged)
    return merged_rows


def _case_id_issues(
    expected_case_ids: list[str],
    reviewed_case_ids: list[str],
) -> list[dict[str, Any]]:
    issues: list[dict[str, Any]] = []
    if not expected_case_ids:
        issues.append({"severity": "error", "issue": "empty_expected_slice"})
    if not reviewed_case_ids:
        issues.append({"severity": "error", "issue": "empty_reviewed_slice"})

    expected_counts = Counter(expected_case_ids)
    reviewed_counts = Counter(reviewed_case_ids)
    duplicate_expected = sorted(
        case_id for case_id, count in expected_counts.items() if count > 1
    )
    duplicate_reviewed = sorted(
        case_id for case_id, count in reviewed_counts.items() if count > 1
    )
    if duplicate_expected:
        issues.append(
            {
                "severity": "error",
                "issue": "duplicate_expected_slice_case_id",
                "case_ids": duplicate_expected,
            }
        )
    if duplicate_reviewed:
        issues.append(
            {
                "severity": "error",
                "issue": "duplicate_reviewed_slice_case_id",
                "case_ids": duplicate_reviewed,
            }
        )

    missing = [case_id for case_id in expected_case_ids if case_id not in reviewed_counts]
    extra = [case_id for case_id in reviewed_case_ids if case_id not in expected_counts]
    if missing:
        issues.append(
            {
                "severity": "error",
                "issue": "missing_reviewed_slice_case_id",
                "case_ids": missing,
            }
        )
    if extra:
        issues.append(
            {
                "severity": "error",
                "issue": "unexpected_reviewed_slice_case_id",
                "case_ids": extra,
            }
        )
    if expected_case_ids and reviewed_case_ids and expected_case_ids != reviewed_case_ids:
        issues.append(
            {
                "severity": "error",
                "issue": "slice_case_order_mismatch",
                "expected": expected_case_ids,
                "actual": reviewed_case_ids,
            }
        )
    return issues


def _slice_row_identity_issues(
    *,
    row: dict[str, str],
    expected_row: dict[str, str],
    row_index: int,
) -> list[dict[str, Any]]:
    issues: list[dict[str, Any]] = []
    for field in ("regime", "degradation_condition", "question_family", "packet_path"):
        expected = str(expected_row.get(field, ""))
        actual = str(row.get(field, ""))
        if expected != actual:
            issues.append(
                {
                    "severity": "error",
                    "issue": "slice_identity_mismatch",
                    "case_id": row.get("case_id", ""),
                    "slice_row_index": row_index,
                    "field": field,
                    "expected": expected,
                    "actual": actual,
                }
            )
    return issues


def _base_workbook_issues(
    *,
    source_rows: list[dict[str, Any]],
    base_workbook_rows: list[dict[str, str]],
) -> list[dict[str, Any]]:
    issues: list[dict[str, Any]] = []
    source_case_ids = [str(row.get("case_id", "")) for row in source_rows]
    workbook_case_ids = [str(row.get("case_id", "")) for row in base_workbook_rows]
    if len(base_workbook_rows) != len(source_rows):
        issues.append(
            {
                "severity": "error",
                "issue": "unexpected_base_workbook_row_count",
                "expected": len(source_rows),
                "actual": len(base_workbook_rows),
            }
        )
    if source_case_ids != workbook_case_ids:
        issues.append(
            {
                "severity": "error",
                "issue": "base_workbook_case_order_mismatch",
                "expected": source_case_ids,
                "actual": workbook_case_ids,
            }
        )
    return issues


def _workbook_row(row: dict[str, Any]) -> dict[str, str]:
    return {field: str(row.get(field, "")) for field in WORKBOOK_COLUMNS}


def write_csv(path: Path, rows: list[dict[str, str]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=WORKBOOK_COLUMNS)
        writer.writeheader()
        writer.writerows(rows)


def write_json(path: Path, payload: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser()
    parser.add_argument("--source-root", default=str(DEFAULT_SOURCE_ROOT))
    parser.add_argument("--expected-slice", default=str(DEFAULT_EXPECTED_SLICE))
    parser.add_argument("--reviewed-slice", default=str(DEFAULT_REVIEWED_SLICE))
    parser.add_argument("--base-workbook", default=str(DEFAULT_BASE_WORKBOOK))
    parser.add_argument("--merged-workbook-out", default="")
    parser.add_argument("--report", default=str(DEFAULT_REPORT))
    return parser


def main() -> int:
    args = build_parser().parse_args()
    source_root = Path(args.source_root)
    reviewed_slice = Path(args.reviewed_slice)
    if not reviewed_slice.exists():
        report = {
            "artifact_kind": "decision_evidence_manuscript_source_review_slice_validation",
            "valid": False,
            "issues": [
                {
                    "severity": "error",
                    "issue": "reviewed_slice_missing",
                    "path": str(reviewed_slice),
                }
            ],
            "issue_counts": {"reviewed_slice_missing": 1},
            "merge_written": False,
            "result_honesty": RESULT_HONESTY,
        }
        write_json(Path(args.report), report)
        print(json.dumps(report, indent=2, sort_keys=True))
        return 1

    merged_workbook_out = str(args.merged_workbook_out).strip()
    base_workbook_rows = read_csv(Path(args.base_workbook)) if merged_workbook_out else None
    source_rows = read_jsonl(source_root / CASE_SOURCE_NAME)
    reviewed_slice_rows = read_csv(reviewed_slice)
    report = validate_slice(
        source_rows=source_rows,
        expected_slice_rows=read_csv(Path(args.expected_slice)),
        reviewed_slice_rows=reviewed_slice_rows,
        base_workbook_rows=base_workbook_rows,
    )

    if report["valid"] and merged_workbook_out:
        assert base_workbook_rows is not None
        merged_rows = merge_slice_into_workbook(
            base_workbook_rows=base_workbook_rows,
            reviewed_slice_rows=reviewed_slice_rows,
        )
        write_csv(Path(merged_workbook_out), merged_rows)
        report = {
            **report,
            "merge_written": True,
            "merged_workbook_out": merged_workbook_out,
            "merged_workbook_row_count": len(merged_rows),
        }
    elif merged_workbook_out:
        report = {**report, "merged_workbook_out": merged_workbook_out}

    write_json(Path(args.report), report)
    print(json.dumps(report, indent=2, sort_keys=True))
    return 0 if report["valid"] else 1


if __name__ == "__main__":
    raise SystemExit(main())
