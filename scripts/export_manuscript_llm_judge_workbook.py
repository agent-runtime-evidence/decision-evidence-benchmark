"""Export a manuscript LLM-judge baseline prediction workbook."""
...
DEFAULT_CSV_OUT = ROOT / "data/results/manuscript_llm_judge_workbook.csv"
DEFAULT_JSONL_OUT = ROOT / "data/results/manuscript_llm_judge_workbook.jsonl"
...
def workbook_rows(
    ...
    *,
    ...
    return [
        _workbook_row(case, baseline=baseline, row_index=index + 1)
...
def redacted_workbook_rows(
    ...
    *,
    ...
    return [
        _redacted_workbook_row(case, baseline=baseline, row_index=index + 1)
        for index, case in enumerate(cases)
    ]


def _workbook_row(case: CaseManifest, *, baseline: str, row_index: int) -> dict[str, str]:
...
def main() -> int:
    ...
    if redacted_input:
        rows = redacted_workbook_rows(case_rows, baseline=str(args.baseline))
        columns = REDACTED_CSV_COLUMNS
    else:
        cases = [CaseManifest.from_dict(row) for row in case_rows]
        rows = workbook_rows(cases, baseline=str(args.baseline))
    ...
    print(
        json.dumps(
            {
                "artifact_kind": "decision_evidence_manuscript_llm_judge_workbook_export",
                ...
                "result_honesty": (
                    "The LLM-judge workbook is an authoring aid with placeholder "
                    "verdicts. It does not create baseline outputs or manuscript results."
                ),
            },
            indent=2,
            sort_keys=True,
